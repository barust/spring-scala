...
class JdbcTemplate(val javaTemplate: org.springframework.jdbc.core.JdbcTemplate) {
	 ...
	 */
	def query[T](sql: String)(extractor: ResultSet => T): T = {
	 ...
	 * @param sql SQL query to execute
	 * @param function function that will extract results, one row at a time
	 * @throws DataAccessException if there is any problem executing the query
	 */
	def query(sql: String)(function: ResultSet => Unit) {
	 ...
	 */
	def query[T](sql: String)(function: (ResultSet, Int) => T): Seq[T] = {
	 ...
	 */
	def queryForObject[T](sql: String)(function: (ResultSet, Int) => T): T = {
